...
function drawGameScreen() {
  ...
  // 顶部按钮安全区：给 Music / 语言 / Home 独立留空间，避免压住对手面板。
  const topControlsH = 36
  const topY = Math.max(SAFE_TOP + topControlsH, 46)

  const availableH = H - topY - actionH - SAFE_BOTTOM - 12
  const panelH = Math.max(150, Math.floor((availableH - centerH - gap * 2) / 2))
  ...
  if (appMode === 'online' && roomId) {
    ...
    drawText(`${t('room')} ${roomId}｜${selfPlayerId === 'p1' ? t('player1') : t('player2')}`, W / 2, Math.max(11, SAFE_TOP - 1), 11, '#fff', 'center', 'bold')
  }
